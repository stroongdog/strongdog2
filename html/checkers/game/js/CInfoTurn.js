function CInfoTurn(iX, iY, iType, oParentContainer){
    ...
    this._init = function(iX, iY, iType, oParentContainer){
        ...
        _oPanel.x = iX;
        _oPanel.y = iY;
        if(s_iGameType === MODE_HUMAN && iType === PAWN_BLACK && s_bMobile && !s_bMultiplayer){
    ...
    this.setName = function(szName){
        var oName =  new createjs.Text(szName," 50px "+PRIMARY_FONT, "#ffffff");
        
        if(_oPanel.y > CANVAS_HEIGHT/2){
            oName.x = -_oBg.getBounds().width/2;
            oName.textAlign = "right";
        } else {
            oName.x = _oBg.getBounds().width/2;
            oName.textAlign = "left";
        }
        oName.textBaseline = "middle";
        oName.lineWidth = 500;
        _oPanel.addChild(oName);
    };
